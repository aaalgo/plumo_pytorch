...
def savenpy(id,filelist,prep_folder,data_path,use_existing=True):      
    ...
    try:
        ...
        np.savez(os.path.join(prep_folder,name+'_crop'), Mask.shape, box, spacing, resolution)
        ...
        info_dic = {'mask_shape':Mask.shape, 'spacing':spacing, 'resolution':resolution, 'extendbox':extendbox}
        pickle.dump(info_dic, open(os.path.join(prep_folder,name+'.pickle'), 'wb'))
...
def full_prep(data_path,prep_folder,n_worker = None,use_existing=True):
    ...
    '''
    filelist = []
    with open('prep_result/list', 'r') as f:
        for l in f:
            filelist.append(l.strip())
    print('to do: %d' %len(filelist))
